...
#define AUTHORS proper_name("Paul Rubin"), proper_name("David MacKenzie")
...
static char const *const total_args[] = {"auto", "always", "only", "never",
                                         nullptr};
static enum total_type const total_types[] = {total_auto, total_always,
...
static bool avx2_supported(void) {
  ...
  if (debug)
    error(0, 0,
          (avx_enabled ? _("using avx2 hardware support")
...
static void write_counts(uintmax_t lines, uintmax_t words, uintmax_t chars,
                         uintmax_t bytes, intmax_t linelength,
                         char const *file) {
  static char const format_sp_int[] = " %*s";
  char const *format_int = format_sp_int + 1;
  char buf[MAX(INT_BUFSIZE_BOUND(intmax_t), INT_BUFSIZE_BOUND(uintmax_t))];
...
static bool wc(int fd, char const *file_x, struct fstatus *fstatus,
               off_t current_pos) {
  int err = 0;
  char buf[IO_BUFSIZE + 1];
  ...
  if (count_bytes && !count_chars && !print_lines && !count_complicated) {
    ...
    if (!fstatus->failed && usable_st_size(&fstatus->st) &&
      ...
      if (end_pos % page_size) {
        ...
        off_t hi_pos = (end_pos - end_pos % (STP_BLKSIZE(&fstatus->st) + 1));
    ...
    for (ssize_t bytes_read;
         ((bytes_read = read(fd, buf + prev, IO_BUFSIZE - prev)) || prev);) {
      if (bytes_read < 0) {
        err = errno;
        break;
      }
      ...
      do {
        ...
        if (!in_shift && 0 <= *p && *p < 0x80) {
          ...
          if (scanbytes < n) {
            if (n == (size_t)-2 && plim - p < IO_BUFSIZE && bytes_read) {
...
static struct fstatus *get_input_fstatus(idx_t nfiles, char *const *file) {
  ...
  if (nfiles == 0 || (nfiles == 1 && ((print_lines + print_words + print_chars +
    ...
    for (idx_t i = 0; i < nfiles; i++)
      fstatus[i].failed =
          (!file[i] || STREQ(file[i], "-") ? fstat(STDIN_FILENO, &fstatus[i].st)
...
ATTRIBUTE_PURE
static int compute_number_width(idx_t nfiles, struct fstatus const *fstatus) {
  int width = 1;
  ...
  for (int i = 0; (file_name = argv_iter(ai, &ai_err)); i++) {
    ...
    if (!file_name[0]) {
      ...
      if (files_from == nullptr)
        ...
        error(0, 0, "%s:%zu: %s", quotef(files_from), argv_iter_n_args(ai),
